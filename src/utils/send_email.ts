...
export async function sendOTPOverEmail(req: Request, res: Response, email: string): Promise<void> {
  try {
    ...
    if (emailSent) {
      res.status(STATUS_CODES.UPDATE_SUCCESS).json(successJson("OTP sent successfully!", null));
    } else {
      res.status(STATUS_CODES.CREATE_FAILURE).json(errorJson("Error in sending email!", null));
    }
  } catch (error) {
    res.status(STATUS_CODES.CREATE_FAILURE).json(errorJson("Error creating OTP record", error));
  }
}
...
export async function verifyOTP(req: Request, res: Response, userEmail: string, otp: number): Promise<void> {
  try {
    ...
    if (!otpDetails || !otpDetails.otp_code) {
      res.status(STATUS_CODES.SELECT_FAILURE).json(errorJson("OTP not found", null));
      return;
    }

    if (parseInt(otpDetails.otp_code) !== otp) {
      res.status(STATUS_CODES.UNAUTHORIZED).json(errorJson("Invalid OTP", null));
      return;
    }
    ...
    res.status(STATUS_CODES.SELECT_SUCCESS).json(successJson("OTP verified successfully", null));
  } catch (error) {
    res.status(STATUS_CODES.SELECT_FAILURE).json(errorJson("Error verifying OTP", error));
  }
}
export async function resetPassword(req: Request, res: Response, email_address: string, password: string): Promise<void> {
  try {
    if (!email_address || !password) {
      res.status(STATUS_CODES.BAD_REQUEST).json(successJson("Email Address and Password required!", null));
      return;
    }
