  ...
  try {
    ...
    if (!role) {
      res
        ...
        .json(errorJson('Role not found', null));
      return;
    }

    res
      .status(STATUS_CODES.SELECT_SUCCESS)
      .json(successJson('Role fetched successfully', role));
  } catch (error) {
    res
      .status(STATUS_CODES.SELECT_FAILURE)
...
export async function getAllRoles(req: Request, res: Response): Promise<void> {
  try {
    ...
    res
      .status(STATUS_CODES.SELECT_SUCCESS)
      .json(successJson('Roles fetched successfully', roles));
  } catch (error) {
    res
      .status(STATUS_CODES.SELECT_FAILURE)
      .json(errorJson('Server error', null));
...
export async function createRole(req: Request, res: Response): Promise<void> {
  try {
    const { name } = req.body;
    const newRole = await prismaClient.role.create({ data: { name: name } });

    res
      .status(STATUS_CODES.CREATE_SUCCESS)
      .json(successJson('Role created successfully', newRole.id));
  } catch (error) {
    res
  ...
  try {
    ...
    if (role.name === 'super_admin') {
      res
        ...
        .json(errorJson('Super admin cannot be deleted!', null));
      return;
    }
    ...
    res
      ...
      .send(successJson('Role Deleted Successfully!', 1));
  } catch (error) {
    res
...
export async function updateRole(req: Request, res: Response): Promise<void> {
  try {
    ...
    res
      ...
      .json(successJson('Role updated successfully', 1));
  } catch (error) {
    if (
      ...
      res
        .status(STATUS_CODES.SELECT_FAILURE)
        .json(errorJson('Role not found', error.message));
    } else {
      res
