...
export async function getTotalAmout(
  packageIds: string[],
  subjectIds: string[],
  prisma: PrismaClient
): Promise<Decimal> {
...
export async function createStudentDetails(
  ...
  try {
    ...
    const newStudentDetail: StudentDetail =
      await prismaClient.studentDetail.create({
        data: {
          ...createRecord,
          fees: {
            create: {
    ...
    res
      ...
      .json(successJson('Record Inserted Successfully', newStudentDetail.id));
  } catch (error) {
    res
      .status(STATUS_CODES.CREATE_FAILURE)
      .json(errorJson('Error creating student record', null));
  }
}

export async function editStudentDetails(
  ...
  if (
    ...
    res
      ...
      .json(errorJson("Student can't edit student_fees!", null));
    return;
  }
  ...
  try {
    ...
    if (!studentDetails) {
      res
        .status(STATUS_CODES.BAD_REQUEST)
        .json(errorJson('Student Detail does not exist!', null));
      return;
    }
    ...
    res
      .status(STATUS_CODES.UPDATE_FAILURE)
      .json(errorJson('Error updating student record', null));
  }
}

// Delete StudentDetail record
export async function deleteStudentDetails(
  req: Request,
  res: Response
): Promise<void> {
  const studentId = req.params.student_id;
  try {
    const deletedStudent: StudentDetail =
      await prismaClient.studentDetail.delete({
        where: { user_id: studentId },
      });
    res
      .status(STATUS_CODES.DELETE_SUCCESS)
      .json(successJson('Record Deleted Successfully', deletedStudent.id));
  } catch (error) {
    res
      .status(STATUS_CODES.DELETE_FAILURE)
      .json(errorJson('Error deleting student record', null));
  }
}
...
export async function getAllStudentDetails(
  ...
  try {
    const studentDetails: StudentDetail[] =
      await prismaClient.studentDetail.findMany();
    res
      .status(STATUS_CODES.CREATE_SUCCESS)
      .json(successJson('Records fetched successfully', studentDetails));
  } catch (error) {
    res
      .status(STATUS_CODES.CREATE_FAILURE)
      .json(errorJson('Error fetching student records', null));
  }
}
...
export async function getStudentDetailById(
  ...
  try {
    ...
    res
      ...
      .json(successJson('Record fetched successfully', studentDetail));
  } catch (error) {
    res
      .status(STATUS_CODES.SELECT_FAILURE)
      .json(errorJson('Error fetching student record', null));
  }
}
